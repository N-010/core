...
public:
    ...
    uint64 getTreasury() const
    {
        return treasury;
    }

    void insertRound(uint32 roundId, const m256i& roundRandom, uint16 operatorCount, uint16 revealedCount, bool finalized, bool rewardsDistributed = false)
    {
        RoundData data{};
...
public:
    ...
    RBeaconChecker* state()
    {
        return (RBeaconChecker*)contractStates[RBEACON_CONTRACT_INDEX];
    }

    void beginTick()
    {
        callSystemProcedure(RBEACON_CONTRACT_INDEX, BEGIN_TICK);
    }
    ...
    static id computeCommitHashForTest(const id& seed, const id& salt, uint32 roundId)
    {
        return computeCommitHash(seed, salt, roundId);
    }
...
static inline id makeUser(uint64 base)
{
    return id(base, base + 1, base + 2, base + 3);
}

static inline id makeSeed(uint64 base)
{
    return id(base + 10, base + 20, base + 30, base + 40);
}

static inline id makeSalt(uint64 base)
{
    return id(base + 100, base + 200, base + 300, base + 400);
}

static inline m256i xorSeeds(const std::vector<id>& seeds)
{
...
static inline m256i computeWinnerScore(const m256i& roundRandom, const id& operatorAddr)
{
...
static inline bool isScoreLess(const m256i& a, const m256i& b)
{
...
TEST(ContractRBeacon, CommitRevealFinalize)
{
    ...
    {
        operators.push_back(makeUser(1000 + i));
        seeds.push_back(makeSeed(2000 + i));
        salts.push_back(makeSalt(3000 + i));
...
TEST(ContractRBeacon, InsufficientReveals)
{
    ...
    const id operatorAddr = makeUser(2000);
    const id seed = makeSeed(2100);
    const id salt = makeSalt(2200);
    ...
    const id caller = makeUser(3000);
    const uint64 fee = 100000;
...
TEST(ContractRBeacon, CommitInvalidPhaseRefunds)
{
    ...
    const id operatorAddr = makeUser(9000);
    const id seed = makeSeed(9100);
    const id salt = makeSalt(9200);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
...
TEST(ContractRBeacon, CommitInsufficientDepositRefunds)
{
    ...
    const id operatorAddr = makeUser(9100);
    const id seed = makeSeed(9200);
    const id salt = makeSalt(9300);
    const uint64 deposit = RBEACON_DEPOSIT - 1;
...
TEST(ContractRBeacon, CommitAlreadyCommittedRefunds)
{
    ...
    const id operatorAddr = makeUser(9200);
    const id seed = makeSeed(9300);
    const id salt = makeSalt(9400);
...
TEST(ContractRBeacon, CommitMaxOperatorsReached)
{
    ...
    const uint32 roundId = 0;
    for (uint32 i = 0; i < RBEACON_MAX_OPERATORS; ++i)
    {
        const id operatorAddr = makeUser(10000 + i);
        const id seed = makeSeed(20000 + i);
        const id salt = makeSalt(30000 + i);
        increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operatorAddr, seed, salt, roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }

    const id extraOperator = makeUser(50000);
    const id extraSeed = makeSeed(51000);
    const id extraSalt = makeSalt(52000);
...
TEST(ContractRBeacon, RevealInvalidPhase)
{
    ...
    const id operatorAddr = makeUser(9300);
    const id seed = makeSeed(9400);
    const id salt = makeSalt(9500);
...
TEST(ContractRBeacon, RevealNotCommitted)
{
    ...
    const id operatorAddr = makeUser(9400);
    const id seed = makeSeed(9500);
    const id salt = makeSalt(9600);
    increaseEnergy(operatorAddr, 1);
    auto revealOut = rb.reveal(operatorAddr, seed, salt);
...
TEST(ContractRBeacon, RevealAlreadyRevealed)
{
    ...
    const id operatorAddr = makeUser(9500);
    const id seed = makeSeed(9600);
    const id salt = makeSalt(9700);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    auto commitOut = rb.commit(operatorAddr, seed, salt, 0);
    EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, RevealHashMismatchRemovesCommit)
{
    ...
    const id operatorAddr = makeUser(9600);
    const id seed = makeSeed(9700);
    const id salt = makeSalt(9800);
    const id wrongSalt = makeSalt(9900);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    const uint64 balanceBefore = getBalance(operatorAddr);
    auto commitOut = rb.commit(operatorAddr, seed, salt, 0);
    EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, RevealHashMismatchWithWrongRoundId)
{
    ...
    const id operatorAddr = makeUser(9650);
    const id seed = makeSeed(9750);
    const id salt = makeSalt(9850);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    const uint64 balanceBefore = getBalance(operatorAddr);
...
TEST(ContractRBeacon, FinalizeForfeitsUnrevealedDeposit)
{
    ...
    const id operatorA = makeUser(9700);
    const id operatorB = makeUser(9800);
    const id seedA = makeSeed(10000);
    const id seedB = makeSeed(10100);
    const id saltA = makeSalt(11000);
    const id saltB = makeSalt(11100);
...
TEST(ContractRBeacon, RoundNotFoundErrors)
{
    ...
    const id caller = makeUser(9900);
    const uint64 fee = 1000;
    increaseEnergy(caller, fee);
    const uint64 balanceBefore = getBalance(caller);
...
TEST(ContractRBeacon, GetRandomWithFeeRoundNotFinalized)
{
    ...
    const uint32 roundId = 7;
    rb.state()->insertRound(roundId, NULL_ID, 3, RBEACON_MIN_REVEALS, false);

    const id caller = makeUser(9950);
    const uint64 fee = 5000;
    increaseEnergy(caller, fee);
    const uint64 balanceBefore = getBalance(caller);
...
TEST(ContractRBeacon, GetRandomWithFeeInsufficientRevealsZero)
{
    ...
    const uint32 roundId = 9;
    rb.state()->insertRound(roundId, NULL_ID, 0, 0, true);

    const id caller = makeUser(9960);
    const uint64 fee = 1000;
    increaseEnergy(caller, fee);
    const uint64 balanceBefore = getBalance(caller);
    auto out = rb.getRandomWithFee(caller, roundId, fee);
...
TEST(ContractRBeacon, GetCurrentRoundCountsAfterCommitReveal)
{
    ...
    const id operatorAddr = makeUser(9970);
    const id seed = makeSeed(9980);
    const id salt = makeSalt(9990);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    auto commitOut = rb.commit(operatorAddr, seed, salt, 0);
    EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, CommitZeroDeposit)
{
    ...
    const id operatorAddr = makeUser(100000);
    const id seed = makeSeed(100100);
    const id salt = makeSalt(100200);
    increaseEnergy(operatorAddr, 1);
    const uint64 balanceBefore = getBalance(operatorAddr);
...
TEST(ContractRBeacon, CommitRoundBoundaryRoundId)
{
    ...
    const id operatorA = makeUser(100300);
    const id seedA = makeSeed(100400);
    const id saltA = makeSalt(100500);
    increaseEnergy(operatorA, RBEACON_DEPOSIT);
    ...
    const id operatorB = makeUser(100600);
    const id seedB = makeSeed(100700);
    const id saltB = makeSalt(100800);
    increaseEnergy(operatorB, RBEACON_DEPOSIT);
...
TEST(ContractRBeacon, RevealPhaseBoundaries)
{
    ...
    const id operatorAddr = makeUser(100900);
    const id seed = makeSeed(101000);
    const id salt = makeSalt(101100);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    auto commitOut = rb.commit(operatorAddr, seed, salt, 0);
    EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, RevealAfterFinalize)
{
    ...
    const id operatorAddr = makeUser(101200);
    const id seed = makeSeed(101300);
    const id salt = makeSalt(101400);
    increaseEnergy(operatorAddr, RBEACON_DEPOSIT);
    auto commitOut = rb.commit(operatorAddr, seed, salt, roundId);
    EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, GetRandomWithFeeZeroFee)
{
    ...
    for (uint64 i = 0; i < 2; ++i)
    {
        operators.push_back(makeUser(101500 + i));
        seeds.push_back(makeSeed(101600 + i));
        salts.push_back(makeSalt(101700 + i));
        increaseEnergy(operators.back(), RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operators.back(), seeds.back(), salts.back(), roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }
    ...
    const id caller = makeUser(101800);
...
TEST(ContractRBeacon, RewardPerWinnerZero)
{
    ...
    {
        operators.push_back(makeUser(101900 + i));
        seeds.push_back(makeSeed(102000 + i));
        salts.push_back(makeSalt(102100 + i));
        increaseEnergy(operators.back(), RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operators.back(), seeds.back(), salts.back(), roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }
    ...
    const id caller = makeUser(102200);
    const uint64 fee = 3;
    increaseEnergy(caller, fee);
    auto out = rb.getRandomWithFee(caller, roundId, fee);
    EXPECT_EQ(out.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, TreasuryFeeSplit)
{
    ...
    for (uint64 i = 0; i < 2; ++i)
    {
        operators.push_back(makeUser(102300 + i));
        seeds.push_back(makeSeed(102400 + i));
        salts.push_back(makeSalt(102500 + i));
        increaseEnergy(operators.back(), RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operators.back(), seeds.back(), salts.back(), roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }
    ...
    const uint64 fee = 1000;
    const uint64 treasuryBefore = rb.state()->getTreasury();
    const id caller = makeUser(102600);
    increaseEnergy(caller, fee);
    auto out = rb.getRandomWithFee(caller, roundId, fee);
    EXPECT_EQ(out.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
...
TEST(ContractRBeacon, RoundHistoryMultipleRounds)
{
    ...
    const uint32 round1 = 1;
    rb.setTick(100, true);

    const id operatorA = makeUser(102700);
    const id seedA = makeSeed(102800);
    const id saltA = makeSalt(102900);
    const id operatorB = makeUser(103000);
    const id seedB = makeSeed(103100);
    const id saltB = makeSalt(103200);
    increaseEnergy(operatorA, RBEACON_DEPOSIT);
    increaseEnergy(operatorB, RBEACON_DEPOSIT);
    ...
    const uint32 round2 = 2;
    rb.setTick(210, false);
    const id operatorC = makeUser(103300);
    const id seedC = makeSeed(103400);
    const id saltC = makeSalt(103500);
    const id operatorD = makeUser(103600);
    const id seedD = makeSeed(103700);
    const id saltD = makeSalt(103800);
...
TEST(ContractRBeacon, RewardsDistributedOnceWithFewReveals)
{
    ...
    for (uint64 i = 0; i < 3; ++i)
    {
        operators.push_back(makeUser(11000 + i));
        seeds.push_back(makeSeed(12000 + i));
        salts.push_back(makeSalt(13000 + i));
        increaseEnergy(operators.back(), RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operators.back(), seeds.back(), salts.back(), roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }
    ...
    const uint64 fee = 90000;
    const uint64 rewardPool = (fee * RBEACON_REWARD_PERCENT) / 100ULL;
    ...
    const id caller = makeUser(14000);
    increaseEnergy(caller, fee);
    auto out = rb.getRandomWithFee(caller, roundId, fee);
    EXPECT_EQ(out.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    EXPECT_EQ(out.randomValue, expectedRandom);
    ...
    auto roundInfo = rb.getRoundInfo(roundId);
    const uint64 totalRewardPool = rewardPool + (secondFee * RBEACON_REWARD_PERCENT) / 100ULL;
...
TEST(ContractRBeacon, TopKRewardsDistribution)
{
    ...
    {
        operators.push_back(makeUser(4000 + i));
        seeds.push_back(makeSeed(5000 + i));
        salts.push_back(makeSalt(6000 + i));
        increaseEnergy(operators.back(), RBEACON_DEPOSIT);
        auto commitOut = rb.commit(operators.back(), seeds.back(), salts.back(), roundId);
        EXPECT_EQ(commitOut.returnCode, static_cast<uint8>(RBEACON::EReturnCode::SUCCESS));
    }
    ...
    std::sort(scored.begin(), scored.end(), [](const ScoredOperator& a, const ScoredOperator& b)
    {
        return isScoreLess(a.score, b.score);
    });
    ...
    const uint64 fee = 200000;
    const uint64 rewardPool = (fee * RBEACON_REWARD_PERCENT) / 100ULL;
    const uint64 rewardPerWinner = rewardPool / RBEACON_K_WINNERS;
